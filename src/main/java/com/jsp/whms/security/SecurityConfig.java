...
import com.jsp.whms.filter.ApiKeyFilter;
...
public class SecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		...
		return httpSecurity.csrf(csrf -> csrf.disable()).securityMatchers(matcher -> {
			...
			matcher.requestMatchers("/api/v1/**","/login/**");
			})
	...
	@Bean
	@Order(1)  //specifies the order of filter chain execution
	SecurityFilterChain ApiKeyFilterChain(HttpSecurity httpSecurity) throws Exception {

		return httpSecurity.csrf(csrf -> csrf.disable())
				.securityMatchers(matcher -> {

					matcher.requestMatchers("/api/v1/client/**");
				})
				.authorizeHttpRequests(authorize ->
				authorize.anyRequest()
				.permitAll()).sessionManagement(session -> 
